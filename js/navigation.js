...
let trackingDeviceOrientation = false; // 是否正在监听设备方向
let lastDeviceHeading = null;         // 设备方向（度，0-360）
let deviceOrientationHandler = null;  // 设备方向事件回调引用
...
function startSimulatedNavigation() {
    ...
    const myIcon = new AMap.Icon({
        ...
        image: iconCfg.icon || MapConfig.markerStyles.currentLocation.icon,
...
function startRealNavigationTracking() {
    ...
    gpsWatchId = navigator.geolocation.watchPosition(
        pos => {
            ...
            // 初始化标记与灰色路径
            if (!userMarker) {
                // 使用带方向的箭头图标（与首页一致），并支持 rotationOffset
                const iconCfg = MapConfig.markerStyles.headingLocation || {};
            ...
            // 计算并设置朝向：优先使用设备方向（若用户允许），否则回退到GPS移动方向
            if (lastDeviceHeading !== null) {
                const iconCfg = MapConfig.markerStyles.headingLocation || {};
                const offsetDeg = iconCfg.rotationOffset || 0;
                const angle = (lastDeviceHeading + offsetDeg) % 360;
                // 使用设备方向（已是0-360）
                try {
                ...
                if (moveDist > 0.5) { // 小于0.5米忽略抖动
                    const bearingRaw = calculateBearingBetweenPoints(lastGpsPos, curr);
                    const iconCfg = MapConfig.markerStyles.headingLocation || {};
                    const offsetDeg = iconCfg.rotationOffset || 0;
                    const bearing = (bearingRaw + offsetDeg) % 360;
                    try {
                        if (typeof userMarker.setAngle === 'function') {
...
// 尝试开启设备方向监听（兼容 iOS 需要的权限请求）
function tryStartDeviceOrientation() {
    if (trackingDeviceOrientation) return;

    // iOS 13+ 需要通过 DeviceOrientationEvent.requestPermission()
    const isIOS = /iP(ad|hone|od)/i.test(navigator.userAgent);
    const startListening = () => {
        deviceOrientationHandler = function(e) {
            if (!e) return;
            // 优先 iOS 的 webkitCompassHeading（已指向正北，0-360）
            let heading = null;
        ...
        trackingDeviceOrientation = true;
    };
    ...
    try {
        if (isIOS && typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
            DeviceOrientationEvent.requestPermission().then(permissionState => {
                if (permissionState === 'granted') {
                    ...
                    console.warn('用户拒绝设备方向权限');
                }
            }).catch(err => {
                console.warn('请求设备方向权限失败:', err);
            });
        } else {
            // 非iOS或无需权限
            startListening();
        ...
        console.warn('开启设备方向监听失败:', e);
    }
}

function tryStopDeviceOrientation() {
    if (!trackingDeviceOrientation) return;
    try {
        ...
        console.warn('移除设备方向监听失败:', e);
    }
    trackingDeviceOrientation = false;
...
function findClosestPathIndex(point, path) {
    ...
    navigationTimer = setInterval(() => {
        ...
        // 更新用户标记位置与朝向
        try {
            const bearingRaw = calculateBearingBetweenPoints(segStart, currPos);
            const iconCfg = MapConfig.markerStyles.headingLocation || {};
            const offsetDeg = iconCfg.rotationOffset || 0;
            const bearing = (bearingRaw + offsetDeg) % 360;
            if (typeof userMarker.setAngle === 'function') {
